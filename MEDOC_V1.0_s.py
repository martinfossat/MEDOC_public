...
def main_prediction(T,neigh,contexts,pat_E,pat_ste,pat_seq,map,prun_during,per_res,base_E,test_time,max_diff,silent=False):
    ...
    for i in range(neigh):
        dim=dim+(2,)
    if not per_res:
        lvl_disc_E=np.zeros(dim,dtype=np.single)
        lvl_disc_E[:]=np.single('+inf')
    ...
    if per_res:
        lvl_context0=np.zeros(dim,dtype=np.single)
        lvl_context0[:]=float('+inf')
        lvl_context1=np.zeros(dim,dtype=np.single)
        lvl_context1[:]=float('+inf')
    else :
        lvl_ste=[np.array(['']) for i in range(len(contexts)+1)]
        
        lvl_E=[np.array([base_E],dtype=np.single) for i in range(len(contexts)+1)]
    tot=0
    ...
    for i in range(len(pat_E)):
        ...
        for t in range(i+1):
            if not per_res:
                temp=np.zeros((len(lvl_ste[t])),dtype=np.single)
                temp[:]=float('nan')
        ...
        temp_pat_E=np.zeros(dim,dtype=np.single)
        ...
        if prun_during and not per_res :
            disc_temp_E=np.zeros(np.shape(lvl_disc_E),dtype=np.single)
            disc_temp_E[:,:]=float('+inf')

        if per_res:
            lvl_temp_context0= np.zeros(np.shape(lvl_context0),dtype=np.single)
            lvl_temp_context0[:,:]=float('+inf')

            lvl_temp_context1= np.zeros(np.shape(lvl_context1),dtype=np.single)
...
def plot_proba_F_per_res(save0,save1,pH,T):
    proba0=np.zeros((len(save0[0]),len(pH)),dtype=np.single)
    proba1=np.zeros((len(save0[0]),len(pH)),dtype=np.single)
    
    for k in range(len(save0[0])):
        W0=np.zeros((len(save0),len(pH)),dtype=np.single)
        W1=np.zeros((len(save0),len(pH)),dtype=np.single)
...
def plot_frac_and_deriv(pH_a,addi,name,points=[[],[]],points_syn=[[],[]],loc='./',prefix='',hill_param=[],fit=True,plot=True):
    ...
    if len(points[0])>0:
        ...
        corr=np.zeros((2,2))*np.single('NaN')
    if plot :
        plt.title(W_title)
        plt.subplot(211)
        if fit:
...
def get_probas_2(Fs,Fs_err,pH,T,ign_norm_err=True,unsafe=0):
    ...
    Weights=np.zeros((len(Fs),len(pH)),dtype=np.single)
    Weights_err=np.zeros((len(Fs),len(pH)),dtype=np.single)
    Weights_norm=np.zeros((len(pH)),dtype=np.single)
    Weights_norm_err=np.zeros((len(pH)),dtype=np.single)
    
    MIN=np.amin(Fs)
    MAX=np.amax(Fs)

    Exponents=np.zeros((len(Fs),len(pH)),dtype=np.single)
    Exponents2=np.zeros((len(Fs),len(pH)),dtype=np.single)
    Exponents3=np.zeros((len(Fs),len(pH)),dtype=np.single)
    Fs=np.asarray(Fs)
...
AA_type=['E','D','H','K','Y','8','9','R','C','A','F','G','L','I','M','N','P','Q','T','V','S','W']
default_res=0.01
R=np.single(1.98720425864083*10**(-3))
...
if __name__=="__main__":
    ...
    pH=np.arange(pH_range[0],pH_range[1],res,dtype=np.single)
    ...
    if det_lvl>=2 and per_res:
        ...
        param_all=np.zeros((len(proba0),3),dtype=np.single)
        names=[]
