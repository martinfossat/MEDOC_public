...
if __name__=="__main__":
    ...
    pH=np.arange(pH_range[0],pH_range[1]+res,res,dtype=np.longdouble)
    ...
    if per_res :
        ...
        for i in range(neigh):
            dim=dim+(2,)
        save_lvl_E=np.zeros((len(pat_E)+1,1),dtype=np.longdouble)
        save_lvl_tot_E=np.zeros((dim),dtype=np.longdouble)
