...
def  initialize_additive_DF_array_MEDOC_public():
    ...
    ,[-0.07447997933624267,0.02916747879908975,0.06389228591248804,0.02170970257759925,0.1308768760056414,-0.022224698687782096]]],dtype=np.single)
    DF_Q=np.array([[[-0.26912017526287124,-0.19790042588058968,-0.1771325119001334,-0.1390228005347286,-0.1157313592250951,-0.10623193041288091]\
        ...
        ,[[0.3293775540187506,0.2913838324984375,0.21159625457985348,0.17397836516386642,0.14959887418322163,0.13085586119282838]\
         ...
         ,[float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN')]\
         ,[float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN')]\
         ,[float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN'),float('NaN')]]],dtype=np.single)
...
def main_prediction(T,neigh,contexts,pat_E,pat_ste,pat_seq,map,prun_during,per_res,base_E,test_time,max_diff,silent=False,state_prun=False):
    ...
    if not per_res:
        ...
        lvl_tot_E=np.zeros(dim,dtype=np.single)
        lvl_tot_E[:]=np.single('+inf')
    ...
    if per_res:
        ...
        lvl_E=[np.array([],dtype=np.single) for i in range(len(contexts)+1)]
        lvl_E[0]=np.append(lvl_E[0],[base_E])
        for q in range(1,len(lvl_E)):
            lvl_E[q]=np.append(lvl_E[q],[np.single("+inf")])
    ############################################################################
    ...
    for t in range(len(pat_E)):
        ...
        if not per_res :
            tot_temp_E=np.zeros(np.shape(lvl_tot_E),dtype=np.single)
...
def compute_charge_density(frac,charge,pH,seq,indices,suffix,win_sz=3,get_back=False):
    ...
    plt.imshow(out.transpose(),vmin=-1,vmax=1,cmap='bwr_r',origin='lower',interpolation=None,aspect='auto')
...
if __name__=="__main__":
    ...
    pH=np.arange(pH_range[0],pH_range[1]+res,res,dtype=np.single)
    ...
    if per_res :
        ...
        for i in range(neigh):
            dim=dim+(2,)
        save_lvl_context0=np.zeros(dim,dtype=np.single)
        save_lvl_context0[:]=float('+inf')
        save_lvl_context1=np.zeros(dim,dtype=np.single)
        save_lvl_context1[:]=float('+inf')
    else :
        dim=(len(contexts)+1,)
        for i in range(neigh):
            dim=dim+(2,)
        save_lvl_E=np.zeros((len(pat_E)+1,1),dtype=np.single)
        save_lvl_tot_E=np.zeros((dim),dtype=np.single)
